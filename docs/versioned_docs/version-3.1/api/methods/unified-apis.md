...
Use `advancedCommerceDataIOS` to pass attribution data (campaign tokens, affiliate IDs) during purchase:
...
- `params` (object):
  - `request` (object): Purchase request configuration
    ...
    - `advancedCommerceDataIOS?` (string, iOS only): Campaign token or attribution data for StoreKit 2's `Product.PurchaseOption.custom` API (iOS 15+)
